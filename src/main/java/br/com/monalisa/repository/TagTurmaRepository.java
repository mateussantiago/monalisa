...
public interface TagTurmaRepository extends JpaRepository<TagTurma, Long>, JpaSpecificationExecutor<TagTurma> {
    @Query(value = "select * from public.tag_turma where ativo is true", nativeQuery = true)
    List<TagTurma> buscarTodos();

    @Query(value = "select * from public.tag_turma where id_tag_turma = :id and ativo is true", nativeQuery = true)
